...
def decode_secret(flat_medium, sec_ext, length, run_code):        # KNOWN BUG: will leave out the last 3 chars, idk why
    '''
...
def decode(stego_img, sec_ext, run_code):
    ...
    decode_secret(img, sec_ext, secret_size, run_code)
...
def decode_images_from_pdf(pdf_path, sec_ext):
    ...
    for idx, img in enumerate(images):
        ...
        decode_secret(img_copy, sec_ext, secret_size, False)
...
if __name__ == "__main__":
    ...
    decode_images_from_pdf(OUTPUT_MEDIA_DIR + "\\" +"test_out.pdf", 'py')
